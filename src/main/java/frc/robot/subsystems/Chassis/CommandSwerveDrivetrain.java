...
public class CommandSwerveDrivetrain extends TunerSwerveDrivetrain implements Subsystem {
        ...
        try {
            ...
            AutoBuilder.configure(
                ...
                new PPHolonomicDriveController(
                    ...
                    new PIDConstants(AutoConstants.followPathTranslationkP, AutoConstants.followPathTranslationkI, AutoConstants.followPathTranslationkD),
                    // PID constants for rotation
                    new PIDConstants(AutoConstants.followPathRotationkP, AutoConstants.followPathRotationkI, AutoConstants.followPathRotationkD)
                ),
                config,
    ...
    public void updateOdometry(){
        ...
        if (Math.abs(getRobotRelativeSpeeds().omegaRadiansPerSecond) <= 4*Math.PI 
            && mt2.tagCount > 0 
            && mt2.avgTagDist < 4 
            && Math.hypot(getRobotRelativeSpeeds().vxMetersPerSecond, getRobotRelativeSpeeds().vyMetersPerSecond) < 2
        ) {
    ...
    public boolean isAtRotation(Rotation2d r){
        return Math.abs(getPose().getRotation().minus(r).getRadians()) <= AutoConstants.moveToPoseRotationToleranceRadians;
    }
    ...
    private ChassisSpeeds optimizeMoveToSpeeds(ChassisSpeeds speeds) {
        ...
        if(Math.hypot(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond) > AutoConstants.maxMoveToSpeed) {
            optimizedSpd = optimizedSpd.times(AutoConstants.maxMoveToSpeed / Math.hypot(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond));
        }
        if(Math.abs(speeds.omegaRadiansPerSecond) > AutoConstants.maxMoveToAngularVelocity) {
            optimizedSpd.omegaRadiansPerSecond = Math.copySign(AutoConstants.maxMoveToAngularVelocity, speeds.omegaRadiansPerSecond);
        }
        return optimizedSpd;
    }
    ...
    public ChassisSpeeds getHybridMoveToPoseSpeeds(Pose2d pose, ImprovedCommandXboxController driverController, double translationAdjustmentRange, double rotationAdjustmentRangeDegs){
        ...
        Transform2d adjustment = new Transform2d(
            driverController.getLeftY()*translationAdjustmentRange * (DriverStation.getAlliance().get() == Alliance.Blue ? -1. : +1.),
     ...
     * <p>Only processes blue originated inputs! If you're on Red, rotate the input around the center.
    ...
    /**
     * Only accepts blue originated inputs! If you're on Red, rotate the input around the center.
    ...
    /**
     * Only accepts blue originated inputs! If you're on Red, use {@link #generateAlignedStationPoseRed(Pose2d)} instead.
     * <p> Generates a pose that is aligned with one of the chutes.
     * For more information, see {@link #generateStationPose(Pose2d)} and {@link #generateStationPose(double)}
     * @param currentPose
     * @return target pose
     */
    public Pose2d generateAlignedStationPoseBlue(Pose2d currentPose){
        Pose2d projection = generateStationPose(currentPose);
        int lowerIndex = (int) Math.floor((projection.getX() - FieldConstants.BlueFirstChuteTranslationX) / FieldConstants.DChuteX);
        int upperIndex = lowerIndex + 1;
        Translation2d t1, t2;
        if(projection.getY() >= FieldConstants.FieldCenter.getY()){
            t1 = FieldConstants.BlueLeftStation.calculate(lowerIndex * FieldConstants.DChuteX + FieldConstants.BlueFirstChuteTranslationX);
            t2 = FieldConstants.BlueLeftStation.calculate(upperIndex * FieldConstants.DChuteX + FieldConstants.BlueFirstChuteTranslationX);
        }
        else {
            t1 = FieldConstants.BlueRghtStation.calculate(lowerIndex * FieldConstants.DChuteX + FieldConstants.BlueFirstChuteTranslationX);
            t2 = FieldConstants.BlueRghtStation.calculate(upperIndex * FieldConstants.DChuteX + FieldConstants.BlueFirstChuteTranslationX);
        }
        Translation2d t = t1.getDistance(projection.getTranslation()) < t2.getDistance(projection.getTranslation())? t1 : t2;
    ...
    public Pose2d generateAlignedStationPoseRed(Pose2d currentPose){
        return FieldConstants.rotateAroundCenter(
            generateAlignedStationPoseBlue(FieldConstants.rotateAroundCenter(getPose(), FieldConstants.FieldCenter, Rotation2d.k180deg)),
            FieldConstants.FieldCenter,
            Rotation2d.k180deg
        );
    }
